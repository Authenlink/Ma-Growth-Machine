...
/**
 * Adapter pour le scraper LinkedIn Profile Posts
 * Scrape les posts d'un profil LinkedIn (lead)
 */
export class LinkedInProfilePostsAdapter extends ApifyAdapter {
  ...
  /**
   * Exécute le scraping des posts de profil LinkedIn
   * @param params Paramètres contenant linkedinUrl (array), maxPosts, postedDateLimit
   */
  async execute(params: Record<string, unknown>): Promise<ScraperRun> {
    // Préparer les paramètres pour Apify
    const input: Record<string, unknown> = {};

    // targetUrls doit être un array d'URLs (le scraper Apify attend targetUrls, pas linkedinUrl)
    let targetUrls: string[] = [];
    if (Array.isArray(params.linkedinUrl)) {
      targetUrls = params.linkedinUrl as string[];
    } else if (params.linkedinUrl) {
      targetUrls = [params.linkedinUrl as string];
    } else {
      throw new Error("linkedinUrl est requis (string ou array)");
    }
